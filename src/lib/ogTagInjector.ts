...
export const injectOGTags = (config: OGTagConfig) => {
  ...
  const metaTags = [
    ...
    keywords.length > 0 ? { name: 'keywords', content: keywords.join(', ') } : null,
    ...
    { property: 'og:site_name', content: 'Saksham Pathfinder' },
...
export const generateInternshipOGTags = (internship: any) => {
  ...
  const location =
    typeof internship.location === 'string' ? internship.location : internship.location?.city || 'India';
  const description = `${internship.title} • ${location} • ${internship.stipend} • Saksham Pathfinder`;
 ...
 */
export const generateComparisonOGTags = (internships: any[], customTitle?: string) => {
  const count = internships.length;
  const companies = internships.map((i) => i.company).slice(0, 2).join(' vs ');
  const title = customTitle || `Compare ${companies} Internships`;
  const description = `Compare ${count} internship opportunity${count > 1 ? 'ies' : ''} on Saksham Pathfinder`;
  ...
  return {
    ...
    url,
    keywords: ['comparison', 'internship', 'opportunity', ...internships.map((i) => i.company)],
  };
};
