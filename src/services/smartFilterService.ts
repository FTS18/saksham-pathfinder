...
export class SmartFilterService {
   ...
   */
  static getPresetFilters(preset: 'high-paying' | 'remote-friendly' | 'skill-focused' | 'location-flexible'): Partial<FilterState> {
    switch (preset) {
      case 'high-paying':
        return {
          minStipend: '15000',
   ...
   */
  static calculateFilterMatchScore(profile: ProfileData, appliedFilters: FilterState): number {
    let score = 0;
    let maxScore = 0;
    ...
    if (profile.interests && profile.interests.length > 0) {
      maxScore += 30;
      if (appliedFilters.selectedSectors && appliedFilters.selectedSectors.length > 0) {
    ...
    if (profile.skills && profile.skills.length > 0) {
      maxScore += 40;
      if (appliedFilters.selectedSkills && appliedFilters.selectedSkills.length > 0) {
  ...
  static getSuggestions(
    ...
    if (resultCount < 5) {
      if (currentFilters.selectedSkills && currentFilters.selectedSkills.length > 3) {
        suggestions.push({
          label: 'Broaden Skills',
          filters: { selectedSkills: currentFilters.selectedSkills.slice(0, 3) },
          reason: 'Show more opportunities by reducing skill requirements'
        });
      }

      if (currentFilters.minStipend !== 'all' && parseInt(currentFilters.minStipend) > 10000) {
        suggestions.push({
    ...
    if (resultCount > 50) {
      if (profile.skills && profile.skills.length > 0 && (!currentFilters.selectedSkills || currentFilters.selectedSkills.length === 0)) {
        suggestions.push({
