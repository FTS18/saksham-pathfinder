...
class UserPreferencesService {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        preferences = {
          ...
          theme: String(profileData?.theme || "dark").replace(
            /[\s\n\r\t]/g,
            ""
          ),
          colorTheme: String(profileData?.colorTheme || "blue").replace(
            /[\s\n\r\t]/g,
            ""
          ),
  ...
  private static getLocalStorageData() {
    const searchHistory = JSON.parse(
      localStorage.getItem("saksham_search_history") || "[]"
  ...
  static async removeFromSearchHistory(
    userId: string,
    query: string
  ): Promise<void> {
    try {
  ...
  static async addToRecentlyViewed(
    userId: string,
    ...
    try {
      ...
      // Remove existing entry if it exists
      const filteredViewed = preferences.recentlyViewed.filter(
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        const mergedWishlist = [
          ...new Set([...(firebaseData.wishlist || []), ...localData.wishlist]),
        ];
        const mergedSearchHistory = [
          ...(firebaseData.searchHistory || []),
          ...localData.searchHistory,
        ].slice(0, 10);
        const mergedRecentlyViewed = [
          ...(firebaseData.recentlyViewed || []),
          ...localData.recentlyViewed,
        ].slice(0, 10);
        ...
        if (firebaseData.colorTheme)
          updateData.colorTheme = firebaseData.colorTheme;
  ...
  static async updateLastLoginTheme(
    userId: string,
    theme: string,
    colorTheme: string
  ): Promise<void> {
